...
PVNAMES = {
    "MOVE_CMD":         MOTOR + "MOVE_CMD",
    "HOME_CMD":         MOTOR + "HOME_CMD",
    "bReset":           MOTOR + "bReset",
    "bHalt":            MOTOR + "bHalt",
...
def move_abs_offset(offset, min_time=2.5, max_velocity=2200.0, max_accel=15000.0, expect_comp=None):
    ...
    PVs["MOVE_CMD"].put(1, timeout=1.0)  # StreamDevice expects a 1 or a command string e.g. "MOVE"
    time.sleep(2.0)
    PVs["MOVE_CMD"].put(0, timeout=1.0)  # If using toggled trigger
...
def exec_home(homepos, homemode, vel=50.0, acc=100.0, dec=100.0):
    ...
    PVs["HOME_CMD"].put(1, timeout=1.0)  # StreamDevice expects a 1 or a string like "HOME"
    time.sleep(2.0)
    PVs["HOME_CMD"].put(0, timeout=1.0)  # If using toggled trigger
...
def batch_move_tests(num_moves=5):
    print(f"[STEP] Running batch of {num_moves} random absolute moves and events")
    for i in range(num_moves):
        ...
        if movetype == 'normal':
            ...
            # Retry with valid params after reset
...
move_abs_offset(150.0)
move_abs_offset(-80.0)
# move_abs_offset(6.0)
...
# print("[STEP] Homing to 12.0 with mode=1")
# exec_home(homepos=12.0, homemode=1)
